...
remove_old_excel.py
===================
One-time utility to delete the old ticket_tracker_2601.xlsx that lives at
the root of /Shared Documents/MJHughes OPEN JOBS/ (before the per-job
subfolder layout was introduced).

Target:
    /Shared Documents/MJHughes OPEN JOBS/ticket_tracker_2601.xlsx
...
TARGET_PATH     = "MJHughes OPEN JOBS/ticket_tracker_2601.xlsx"
...
def main() -> None:
    ...
    if item is None:
        logging.info("File not found - nothing to delete")
        sys.exit(0)

    # Safety check: never delete a folder
    if "folder" in item:
        logging.error(
            f"Target path resolves to a folder, not a file. Aborting."
        )
        sys.exit(1)
